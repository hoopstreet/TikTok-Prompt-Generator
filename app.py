...
class TikTokAIGenerator:
    ...
    def generate_taglish_script(self, niche, duration, product_name):
        ...
        for i in range(shots):
            ...
            scripts.append(f"Shot {i+1:02d} (3s): [4K vertical 9:16] [Product showcase] {taglish}")
        return scripts
# PHASE 1: DNS AND PULL LATEST
echo "nameserver 8.8.8.8" > /etc/resolv.conf
cd ~/TikTok-Prompt-Generator
git pull origin main

# PHASE 2: START app.py
cat << 'EOF' > app.py
import gradio as gr
import random
import re
...
low quality, blurry, distorted, glitch, color bleed, watermark, low resolution, messy textures, robotic voice, formal language, text overlap, no motion blur, no frame tearing

================================================================================

Column: final_title

🔥 MUST-HAVE! {title[:50]} | {niche.upper()} Finds #TikTokMadeMeBuyIt #BudolFinds #Sulit #Quality #Affiliate
...
with gr.Blocks(title="TikTok Prompt Generator", theme="soft") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            submit = gr.Button("Generate TikTok Script", variant="primary")
